...
final rankingControllerProvider = StateNotifierProvider<RankingController, bool>(
  (ref) => RankingController(ref: ref),
);
...
final getFriendsLeaderboard = FutureProvider<List<UserModel>>((ref) async {
  final _controller = ref.watch(rankingControllerProvider.notifier);
  return await _controller.getFriendsRank();
});

class RankingController extends StateNotifier<bool> {
  final Ref _ref;
  RankingController({required Ref ref}) : _ref = ref, super(false);
  ...
  Future<List<UserModel>> getFriendsRank() async {
    try {
      return await _repo.getFriendsRank();
    } catch (e) {
      log(e.toString());
      rethrow;
    }
